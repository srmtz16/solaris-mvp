...
function Header({ id }: { id: string }) {
  return <header className="border-b border-stone-200/80 bg-[#faf9f6]/90 backdrop-blur-xl">
    <div className="mx-auto flex max-w-6xl items-center justify-between px-5 py-4 md:px-8">
      <div className="flex items-center gap-3"><div className="grid size-10 place-items-center rounded-full bg-[#171713] text-[#d5b66f]"><Sun className="size-5" /></div><div><div className="text-sm font-semibold tracking-[.18em] text-stone-900">SOLARIS</div><div className="text-[10px] uppercase tracking-[.2em] text-stone-500">Cuidado solar</div></div></div>
...
function QuickActions({ notify }: { notify: (message: string) => void }) {
  const actions = [{ label: "Solicitar mantenimiento", icon: CalendarDays }, { label: "Reportar una falla", icon: Headphones }, { label: "Ver historial", icon: History, target: "historial" }, { label: "Ver documentos", icon: FolderOpen, target: "documentos" }];
  return <section id="soporte" className="scroll-mt-8"><SectionHeading eyebrow="Accesos directos" title="¿Qué necesitas?" /><div className="grid grid-cols-2 gap-3 lg:grid-cols-4">{actions.map(({ label, icon: Icon, target }) => <button key={label} onClick={() => target ? scrollTo(target) : notify("Próximamente")} className="flex min-h-32 flex-col justify-between rounded-3xl border border-stone-200 bg-white p-5 text-left transition hover:-translate-y-0.5 hover:border-stone-300 hover:shadow-lg"><Icon className="size-5 text-[#9b7835]" /><span className="mt-6 text-sm font-semibold leading-5 text-stone-900">{label}</span></button>)}</div></section>;
}
...
function MobileNavigation() { const items = [{ label: "Inicio", icon: Home, target: "inicio" }, { label: "Historial", icon: History, target: "historial" }, { label: "Documentos", icon: FolderOpen, target: "documentos" }, { label: "Soporte", icon: Headphones, target: "soporte" }]; return <nav className="fixed inset-x-3 bottom-3 z-40 rounded-2xl border border-stone-200/80 bg-white/95 p-1.5 shadow-[0_12px_40px_rgba(20,20,15,.16)] backdrop-blur-xl md:hidden"><div className="grid grid-cols-4">{items.map(({ label, icon: Icon, target }) => <button key={target} onClick={() => scrollTo(target)} className="flex min-h-12 flex-col items-center justify-center gap-1 rounded-xl text-[10px] font-medium text-stone-500 transition active:bg-stone-100 active:text-stone-900"><Icon className="size-4" />{label}</button>)}</div></nav>; }

export function SystemPortal({ system }: { system: SolarSystem }) {
  const [toast, setToast] = useState("");
  const notify = (message: string) => { setToast(message); window.setTimeout(() => setToast(""), 2200); };
  return <div className="min-h-screen bg-[#faf9f6] text-stone-900"><Header id={system.id} /><main className="mx-auto max-w-6xl space-y-16 px-5 pb-32 pt-10 md:px-8 md:pb-16 md:pt-16"><SystemSummary system={system} /><NextMaintenance system={system} notify={notify} /><MaintenanceHistory system={system} /><Documents system={system} notify={notify} /><Observations system={system} /><QuickActions notify={notify} /></main><footer className="border-t border-stone-200 bg-white px-5 py-10 text-center text-xs text-stone-400"><div className="mb-2 flex items-center justify-center gap-2 font-semibold tracking-[.16em] text-stone-700"><ShieldCheck className="size-4 text-[#9b7835]" /> SOLARIS</div>Expediente de mantenimiento · {system.id}</footer><MobileNavigation />{toast && <div role="status" className="fixed bottom-24 left-1/2 z-50 -translate-x-1/2 rounded-full bg-stone-900 px-5 py-3 text-sm font-medium text-white shadow-xl md:bottom-8">{toast}</div>}</div>;
}
