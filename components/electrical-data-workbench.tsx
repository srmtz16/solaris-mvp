...
import {
  ...
  importElectricalWorkbook, mergeElectricalDatasets, readingsToChartData, summarizeAnalysis, toLocalInput, updateColumnMapping,
...
export function ElectricalDataWorkbench() {
  ...
  async function loadFiles(files: File[], append = false) {
    ...
    if (files.some((file) => !/\.(xls|xlsx|csv)$/i.test(file.name))) { setStatus("error"); setMessage("Algún archivo no es compatible. Usa XLS, XLSX o CSV."); return; }
    if (files.some((file) => file.size > 35 * 1024 * 1024)) { setStatus("error"); setMessage("Uno de los archivos supera 35 MB. Divide esa exportación en periodos más cortos."); return; }
    setStatus("loading"); setMessage(""); setStep("review");
    try {
      const importedFiles = await Promise.all(files.map(async (file) => importElectricalWorkbook(XLSX.read(await file.arrayBuffer(), { type: "array", cellDates: true }), file.name)));
      const imported = mergeElectricalDatasets(append && dataset ? [dataset, ...importedFiles] : importedFiles);
      setDataset(imported); setFilters(defaultFilters(imported));
      const preferred = imported.columns.filter((column) => column.recognized && column.family !== "other").slice(0, 8).map((column) => column.targetId);
  ...
  const compatibleColumns = availableColumns.filter((column) => (!filters?.families.length || filters.families.includes(column.family)) && (!filters?.phases.length || filters.phases.includes(column.phase)));
  const compatibleIds = new Set(compatibleColumns.map((column) => column.targetId));
  const compatibleSelection = selectedVariables.filter((id) => compatibleIds.has(id));
  const effectiveSelectedVariables = compatibleSelection.length ? compatibleSelection : compatibleColumns.slice(0, 8).map((column) => column.targetId);
  const chartReadings = aggregated.filter((reading) => effectiveSelectedVariables.includes(reading.variableId));
  const chartData = readingsToChartData(chartReadings);
  const chartColumns = compatibleColumns.filter((column) => effectiveSelectedVariables.includes(column.targetId));
    ...
    <div className="mx-auto grid min-h-[70vh] max-w-5xl place-items-center">
      <Card className="w-full overflow-hidden border-primary/20">
        ...
        <CardContent className="grid gap-5 p-6 md:grid-cols-[1fr_auto] md:items-center md:p-10">
          ...
          <input ref={inputRef} type="file" multiple className="hidden" accept=".xls,.xlsx,.csv" onChange={(event) => { const files = [...(event.target.files ?? [])]; if (files.length) loadFiles(files); event.currentTarget.value = ""; }} />
  ...
  return <div className="flex flex-col gap-6">
    <div className="flex flex-col justify-between gap-4 lg:flex-row lg:items-end">
      <div><div className="flex items-center gap-2 text-xs font-semibold uppercase tracking-[.18em] text-primary"><Activity className="size-4" />Análisis activo</div><h1 className="mt-2 text-3xl font-semibold">{dataset.fileName}</h1><p className="mt-2 text-sm text-muted-foreground">{dataset.fileNames.length} {dataset.fileNames.length === 1 ? "archivo" : "archivos"} · {dataset.rowCount.toLocaleString("es-MX")} registros · {dataset.columns.length} columnas · datos locales</p></div>
      <div className="flex flex-wrap gap-2"><input ref={inputRef} type="file" multiple className="hidden" accept=".xls,.xlsx,.csv" onChange={(event) => { const files = [...(event.target.files ?? [])]; if (files.length) loadFiles(files, true); event.currentTarget.value = ""; }} /><Button variant="outline" onClick={() => inputRef.current?.click()} disabled={status === "loading"}>{status === "loading" ? <Loader2 className="size-4 animate-spin" /> : <Upload className="size-4" />}Agregar archivos</Button><Button variant="outline" onClick={() => { setDataset(null); setFilters(null); setMessage(""); }}><RefreshCw className="size-4" />Reiniciar</Button>{step === "analysis" ? <><Button variant="outline" onClick={exportCsv}><Download className="size-4" />CSV filtrado</Button><Button onClick={exportPdf}><FileText className="size-4" />Reporte PDF</Button></> : null}</div>
    </div>
    ...
    {step === "analysis" && filters && summary ? <>
      ...
      <Card><CardHeader><CardTitle>Series temporales</CardTitle><CardDescription>Las fases se combinan con lógica “cualquiera de las seleccionadas”. Si cambias de fase, se muestran automáticamente variables compatibles.</CardDescription></CardHeader><CardContent><div className="mb-4 flex max-h-28 flex-wrap gap-2 overflow-auto">{compatibleColumns.map((column) => <Toggle key={column.targetId} active={effectiveSelectedVariables.includes(column.targetId)} onClick={() => setSelectedVariables((current) => current.includes(column.targetId) ? current.filter((id) => id !== column.targetId) : current.length < 8 ? [...current, column.targetId] : current)}>{column.displayName}</Toggle>)}</div>{chartData.length && chartColumns.length ? <ElectricalDataChart data={chartData} columns={chartColumns} zoom={zoom} onZoom={setZoom} /> : <div className="grid h-72 place-items-center rounded-md bg-muted/40 text-sm text-muted-foreground">No hay variables para esta combinación de filtros.</div>}</CardContent></Card>
